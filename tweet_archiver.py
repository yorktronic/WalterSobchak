...
def storeTweetsJson(tweets, screen_name, clean=False):
    # In: list of tweet objects, twitter user screen_name, whether or not tweet has been cleaned
    ...
    if clean:
        with open('./tweets/{}_clean.json'.format(screen_name), 'w') as outFile:
            json.dump(outTweets, outFile)
    # Otherwise just save them with the screen_name
    else:
        with open('./tweets/{}.json'.format(screen_name), 'w') as outFile:
            json.dump(outTweets, outFile)

    print "...{} tweets by {} saved as {}".format(len(tweets), screen_name, outFile)

def cleanTweet(tweet):
    # In: text from a tweet object
    # Out: tokenized list of strings with stopwords removed
    ...
    def removeStopwords(tokens):
        # In: list of tokenized strings
        # Out: list of tokenized strings with stopwords like "in," "an," removed
